import xlsxwriter
import pandas as pd
from Net_Detection import NetDataPrcoess
import xlwt
...
class savedata():
    ...
    def save_data(self, pwd, label_data, label_label):
        pwd_data = pwd + '\\' + 'data'
        # 创建一个workbook 设置编码
        dataprocess = NetDataPrcoess.DataPrcoess()
        dataprocess.setDir(pwd_data)
        ...
        workbook.save(pwd_data + '\\' + 'show_data.xls')
        data = pd.read_excel(pwd_data + '\\' + 'show_data.xls', index_col=0)
        data.to_csv(pwd_data + '\\' + 'show_data.csv', encoding='utf-8')

    def save_excel(self, pwd):  # 储存为excel
        dataprocess = NetDataPrcoess.DataPrcoess()
        pwd_NetData = pwd + '\\' + "NetData"
        dataprocess.setDir(pwd_NetData)
        # print("1")
        ...
        data_data = self.data
        # 返回
        show_data, show_label = dataprocess.writeabnormaldata(data_data, pwd_NetData)
        ...
        data = dataprocess.getdata(pwd_NetData)
        ...
        data.to_csv(self.filename + ".csv", encoding='utf-8')
        # 保存用于最后用于显示的文件
        self.save_data(pwd, show_data, show_label)
