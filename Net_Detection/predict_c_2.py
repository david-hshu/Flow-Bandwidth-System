...
import pandas as pd
import torch
from Net_Detection import nets as my_new_nets
...
class predict():
    def __init__(self, pwd, path):
        ...
        # 加载最佳参数
        self.model.load_state_dict(torch.load("D:/flaskProject/Net_Detection/model/pridect/model_best.pth.tar")['state_dict'])
        ...
        self.model2.load_state_dict(torch.load("D:/flaskProject/Net_Detection/model/pridect_tpcnn/model_best.pth.tar")['state_dict'])
        self.model2.eval()
        self.predicted_all2 = []
        # self.predicted2=[]
        ...
        self.model3.load_state_dict(torch.load("D:/flaskProject/Net_Detection/model/pridect_Pccn/model_best.pth.tar")['state_dict'])
        self.model3.eval()
        self.predicted_all3 = []
        # 为页面展示制作所建
    ...
    def save_excel(self, ):
        ...
        workbook.save(self.pwd + '\\' + 'data/show_label.csv')
        data = pd.read_excel(self.pwd + '\\' + 'data/show_label.csv', index_col=0)
        data.to_csv(self.pwd + '\\' + 'data/show_label.csv', encoding='utf-8')

    # model1
    def finalmainmodel1(self, ):
        ...
        for i in range(1, (len(file) // 64) + 2):
            ...
            if (i * 64) < len(file):
                ...
                predict_data = file.values[(len(file) - 64 if len(file) - 64 > 0 else 0):len(file), 1:]
    ...
    def statistic(self, ):
        ...
        self.save_excel()
        return count0, count1, self.save_flow_member, self.save_flow_name

    def judgment(self, pmodel1, pmodel2, pmodel3):
        if (pmodel1 == pmodel2 == pmodel3 and pmodel1 == 0) or (pmodel1 == pmodel2 and pmodel1 == 0) or (
                pmodel1 == pmodel3 and pmodel3 == 0) or (pmodel2 == pmodel3 and pmodel2 == 0):
            return 0
        else:
            return 1
